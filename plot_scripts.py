...
def initialize_animation1(x_field, y_field, z_field, x_auv, vmin, vmax, var_min, var_max, levels, PlotField, LabelVertices):
    ...
    c22 = plt.contourf(x_field, y_field,
                        np.dot(np.diag(np.linspace(var_min, var_max, len(y_field), endpoint=True)),
                        np.ones((len(y_field), len(x_field)))), 10, vmin=var_min, vmax=var_max)
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    plt.colorbar(c22)
...
def update_animation1(pi_theta, fig1, x, y, bottom, colors, x_field, y_field, z_field, x_auv, mue_x, var_x, params, trajectory_1, tau_x, tau_optimal, vmin, vmax, var_min, var_max, levels, PlotField, LabelVertices):
