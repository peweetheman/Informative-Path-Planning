...
for iter in range(2):
	...
	filename = 'PRM*_data' + str(iter) + '.txt'
	...
	for time_in_ms in range(0, Config.simulation_end_time):  # 1200 ms
		if time_in_ms % Config.sample_time_gmrf < 0.0000001:
			...
			PRM_star = PRM_star_Dubins(start=x_auv, space=[0, 10, 0, 5], obstacles=None, var_x=var_x, gmrf_params=gmrf1.params, max_dist=max_path_length-path_length)
			path_optimal, u_optimal, tau_optimal, dubins_time, method_time = PRM_star.control_algorithm()
			print("dubins planner time: ", dubins_time)
			print("time in shape function method: ", method_time)
			x_auv = tau_optimal[:, 3]
			tau_x = None

			# RRT_star1 = RRT_star(start=x_auv, space=[0, 10, 0, 5], obstacles=None, var_x=var_x, gmrf_params=gmrf1.params, max_dist=max_path_length-path_length)
			# path_optimal, u_optimal, tau_optimal, dubins_time, method_time = RRT_star1.control_algorithm()
			# print("dubins planner time: ", dubins_time)
			# print("steer time: ", method_time)
			# x_auv = tau_optimal[:, 3]
			# tau_x = None
